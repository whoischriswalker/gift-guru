...
module.exports = {
  ...
  getWishListHistory: function (userId, done) {
    var wishListQuery = 'SELECT items.desc, items.url, items.price, wishes.wishId, wishes.userId, IF(wishes.mustHave, TRUE, null) mustHave,  IF(wishes.purchaseStatus, TRUE, null) purchaseStatus, wishes.createDate FROM wishes INNER JOIN items ON wishes.itemId = items.itemId WHERE wishes.createDate < "2023-10-01 00:00:00" AND wishes.createDate > "2023-01-01 00:00:00" AND wishes.userId = ? ORDER BY ?? DESC, ?? DESC'
