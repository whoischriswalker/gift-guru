...
module.exports = {
  getWishList: function (userId, done) {
    var wishListQuery = 'SELECT items.desc, items.url, items.price, wishes.wishId, wishes.mustHave, wishes.status, wishes.createDate FROM wishes INNER JOIN items ON wishes.itemId = items.itemId WHERE wishes.userId = ? ORDER BY ?? DESC'

    pool.query(wishListQuery, [userId, 'wishes.createDate'], function (err, wishes) {
